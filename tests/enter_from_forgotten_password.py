...
"""В данном кейсе проверяем вход из раздела Восстановить Пароль"""


def test_check_enter_to_account_from_forgotten_password_true():
    ...
    WebDriverWait(driver, 5).until(expected_conditions.visibility_of_element_located(
        TestLocators.ENTER_TO_ACCOUNT_BUTTON))#ожидаем появление элемента
    driver.find_element(*TestLocators.ENTER_TO_ACCOUNT_BUTTON).click()  # нажимаем по кнопке войти в аккаунт
    driver.find_element(*TestLocators.RECOVER_PASSWORD_BUTTON).click()# нажимаем по кнопке Восстановить Пароль
    driver.find_element(*TestLocators.ENTER_FROM_RECOVERING_BUTTON).click()# нажимаем по кнопке Войти
